...
trainset = torchvision.datasets.CIFAR10(root='/nfs/home/dgranziol/kfac-curvature/data/cifar10/', train=True, download=True, transform=transform_train)
trainloader = torch.utils.data.DataLoader(trainset, batch_size=128, shuffle=True, num_workers=2)

testset = torchvision.datasets.CIFAR10(root='/nfs/home/dgranziol/kfac-curvature/data/cifar10/', train=False, download=True, transform=transform_test)
...
args.dir = args.dir  + 'CIFAR10/' + args.model + '/SGD'
thing = ""
if args.no_schedule:
    thing += "_flat"
...
if args.normalized_wd:
    weight_decay = args.wd * np.sqrt(args.batch_size / (args.epochs * len(trainloader)))
else:
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.swag and args.swag_c_epochs < 1 and epoch >= args.swag_start:
        ...
        train_res = utils.train_epoch(trainloader, model, criterion, optimizer, verbose=args.verbose,
                                      swag_model=swag_model, swag_batch_c=int(len(trainloader) * args.swag_c_epochs))
    else:
        train_res = utils.train_epoch(trainloader, model, criterion, optimizer, verbose=args.verbose)

    # update batch norm parameters before testing
    utils.bn_update(trainloader, model)
    test_res = utils.eval(testloader, model, criterion)
    ...
    if args.swag and (epoch + 1) > args.swag_start:
        ...
        if epoch == 0 or epoch % args.eval_freq == args.eval_freq - 1 or epoch == args.epochs - 1:
            swag_model.set_swa()
            utils.bn_update(trainloader, swag_model)
            train_res_swag = utils.eval(trainloader, swag_model, criterion, optimizer)
            swag_res = utils.eval(testloader, swag_model, criterion)
