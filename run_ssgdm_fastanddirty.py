...
parser.add_argument('--type', type=str, default='minimum', metavar='type',
                    help='type of ssgdm')
...
#     thing += "_step"
if args.swag:
    thing += '_swastart='+str(args.swag_start)
args.dir += '/' + piu + 'seed='+str(args.seed)+'_epoch_freq='+str(args.epochfreq)+'_curvaturesize='+str(args.curvaturebatchsize)+'_warmstart='+str(args.warmstart)+ '_wd=' + str(
    args.wd) + thing +'_numepochs=' + str(args.epochs) + '/'
...
for epoch in range(start_epoch, args.epochs):
    ...
    if epoch >= args.warmstart and epoch % args.epochfreq == 0:
        ...
        if args.type == 'minimum':
            ...
            print('median')
            mu = np.median(eigvals)

        print(eigvals)
        lr = np.power(2./(np.sqrt(np.amax(eigvals))+np.sqrt(mu)),2)
        print('max eigval = '+str(np.amax(eigvals)))
        print('median eigval = ' + str(np.median(eigvals)))
        momentum = np.power(((np.sqrt(np.amax(eigvals))-np.sqrt(mu)) / (np.sqrt(np.amax(eigvals))+mu)),2)
        print('learning rate')
        print(lr)
        print('momentum')
        print(momentum)
