...
imagenet_train = torchvision.datasets.ImageNet('data/imagefolder/',split='train', transform=transform_train, download=None)
imagenet_val = torchvision.datasets.ImageNet('data/imagefolder/',split='val',  transform=transform_test, download=None)
data_loader_train = torch.utils.data.DataLoader(imagenet_train,
                                          batch_size=128,
                                          shuffle=True,
                                          num_workers=4)
data_loader_val = torch.utils.data.DataLoader(imagenet_val,
                                          batch_size=128,
                                          shuffle=False,
                                          num_workers=4)
# convert types
...
args.dir = args.dir  + 'ImageNet/' + args.model + '/SGD'
thing = ""
if args.no_schedule:
    thing += "_flat"
...
if args.normalized_wd:
    weight_decay = args.wd * np.sqrt(args.batch_size / (args.epochs * len(data_loader_train)))
else:
...
if args.resume is not None:
    ...
    # test_res = utils.eval(data_loader_val, model, criterion)
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.swag and args.swag_c_epochs < 1 and epoch >= args.swag_start:
        ...
        train_res = utils.train_epoch(data_loader_train, model, criterion, optimizer, verbose=args.verbose,
                                      swag_model=swag_model, swag_batch_c=int(len(data_loader_train) * args.swag_c_epochs))
    else:
        train_res = utils.train_epoch(data_loader_train, model, criterion, optimizer, verbose=args.verbose)

    # update batch norm parameters before testing
    utils.bn_update(data_loader_train, model)
    test_res = utils.eval(data_loader_val, model, criterion)
    ...
    if args.swag and (epoch + 1) > args.swag_start:
        ...
        if epoch == 0 or epoch % args.eval_freq == args.eval_freq - 1 or epoch == args.epochs - 1:
            swag_model.set_swa()
            utils.bn_update(data_loader_train, swag_model)
            train_res_swag = utils.eval(data_loader_train, swag_model, criterion, optimizer)
            swag_res = utils.eval(data_loader_val, swag_model, criterion)
