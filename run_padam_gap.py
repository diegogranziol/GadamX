...
args.dir = args.dir + args.dataset + '/' + args.model + '/Padam_nogap' + add + '/seed=' + str(args.seed) + '_lr=' + str(
    args.lr_init) + '_wd=' + str(args.wd) + end + '/'
...
def schedule(epoch, total_epoch):
    ...
    lr_ratio = args.swag_lr / args.lr_init if (args.swag and not args.schedule_swag_lr) else 0.01
    if t <= 0.5:
        factor = 1.0
    elif t <= 0.9:
...
for epoch in range(start_epoch, args.epochs):
    ...
    if epoch >= args.swag_start:
        ...
        for input, target in loaders['train']:
            ...
            # if t % args.swa_gap == 0:
            for param, swag_param in zip(model.parameters(), swag_model.parameters()):
                swag_param.data += (param.data - swag_param.data) / (n_swa + 1)
            n_swa += 1
