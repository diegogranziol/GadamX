...
parser.add_argument('--ckpt', type=str,
                    default="/home/xwan/PycharmProjects/kfac-curvature/out/VGG16/SGD_noschedule/checkpoint-00300.pt",
                    metavar='CKPT', required=False,
                    help='checkpoint to load model (default: None)')
parser.add_argument('--swag', action='store_true')

parser.add_argument('--iters', type=int, default=5, metavar='N', help='number of lanczos steps (default: 20)')
...
#parser.add_argument('--spectrum_path', type=str,
                    #default="/home/xwan/PycharmProjects/kfac-curvature/out/VGG16/SGD_noschedule/spectrum-00300",
                    #metavar='PATH',
...
class CurvVecProduct(object):
    ...
    def __call__(self, vector):
        ...
        elif self.curvature_matrix == 'nonggn':
            output = utils.nonggn_vec(
                vector,
                self.loader,
                self.model,
                self.criterion,
...
if not args.weight_only:
    ...
    if args.curvature_matrix not in ['gn_diag', 'hessian_diag', 'gn_diag_mc']:
        if args.curvature_matrix in ['gn','hessian', 'covgrad','nonggn']:
    ...
    if args.bn_train_mode_off:
        addbn = '_bn_train_mode_off'
    else:
        addbn = ''
    if args.num_samples is not None:
        import os
        import sys
        print('Preparing directory %s' % args.ckpt[:val]+'b='+str(args.num_samples)+'_matrix='+args.curvature_matrix+'/')
        os.makedirs(args.ckpt[:-val]+'b='+str(args.num_samples)+'_matrix='+args.curvature_matrix+'/' , exist_ok=True)
        with open(os.path.join(args.ckpt[:-val]+'b='+str(args.num_samples)+'_matrix='+args.curvature_matrix+'/', 'command.sh'), 'w') as f:
            f.write(' '.join(sys.argv))
            f.write('\n')
        spectrum_path = args.ckpt[:-val] +'b='+str(args.num_samples)+'_matrix='+args.curvature_matrix+'/' +str(int(args.ckpt[-8:-3]))+'_seed='+str(args.subsample_seed)+addbn
        basis_path = args.ckpt[:-val] +'b='+str(args.num_samples)+'_matrix='+args.curvature_matrix+'/' +str(int(args.ckpt[-8:-3]))+'_seed='+str(args.subsample_seed)+addbn
        if args.swag:
            spectrum_path += '_swa'
            basis_path += '_swa'
        else:
            spectrum_path += '_iterate'
            basis_path += '_iterate'
        np.savez(
            spectrum_path,
        ...
        spectrum_path = args.ckpt[:-val]+ args.curvature_matrix+'_'+str(int(args.ckpt[-8:-3]))+addbn
        basis_path = args.ckpt[:-val]+ args.curvature_matrix+'_'+str(int(args.ckpt[-8:-3]))+addbn
        if args.swag:
            spectrum_path += '_swa'
            basis_path += '_swa'
        else:
