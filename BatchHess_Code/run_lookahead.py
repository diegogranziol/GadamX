...
parser = argparse.ArgumentParser(description='Lookahead training')
...
parser.add_argument("--optimizer", type=str, default='sgd', help='Base Optimizer')
parser.add_argument('--alpha', type=float, default=0.5, help='Alpha of Lookahead Optimizer')
parser.add_argument('--k', type=int, default=5, help='Number of steps to look ahead (k parameter in lookahead)')
parser.add_argument('--stop_at_swa', action='store_true', help='Whether to stop lookahead operation after IA point')
# If using Padam
parser.add_argument('--partial', type=float, default=0.125, help="Partially adaptive coefficient. "
                                                                 "Default to 1/8. Only applicable if base optimiser "
                                                                 "is Padam")
...
args.dir = args.dir + args.dataset + '/' + args.model + '/Lookahead' + args.optimizer
thing = ""
if args.no_schedule:
    thing += "_flat"
...
args.dir += '/seed=' + str(args.seed) + '_lr=' + str(args.lr_init) + thing + '_mom=' + str(
    args.momentum) + '_wd=' + str(
...
def schedulestupid(epoch, total_epoch):
    t = epoch / (args.swag_start if (args.swag and not args.schedule_swag_lr) else total_epoch)
    lr_ratio = args.swag_lr / args.lr_init if (args.swag and not args.schedule_swag_lr) else args.lr_r
    if t <= 0.5:
        factor = 1.0
...
if args.optimizer == 'sgd':
    ...
    raise NotImplementedError("Optimizer" + str(args.optimizer) + " as a base optimizer is not implemented here.")

# Initialise the look ahead optimizer
optimizer = Lookahead(
    base_optimizer,
    k=args.k,
    alpha=args.alpha
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.swag and args.swag_c_epochs < 1 and epoch >= args.swag_start:
        if lookahead_enabled is True and args.stop_at_swa:
        ...
        train_res = utils.train_epoch(loaders['train'], model, criterion, optimizer, verbose=args.verbose,
                                      ...
                                      swag_batch_c=int(len(loaders['train']) * args.swag_c_epochs))
    else:
        train_res = utils.train_epoch(loaders['train'], model, criterion, optimizer, verbose=args.verbose)
