...
from curvature import data_subset as data
...
args.dir += '/ss'+ '/seed='+str(args.seed)+'_lr=' + str(args.lr_init) + thing + '_mom=' + str(args.momentum) + '_wd=' + str(
    args.wd) + '_size=' +str(args.sub_sample_size)+'_seed='+str(args.sub_sample_seed)+'_numepochs=' + str(args.epochs) + '/'
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.swag and (epoch + 1) > args.swag_start and (epoch + 1 - args.swag_start) % args.swag_c_epochs == 0:
        ...
        if epoch == 0 or epoch % args.eval_freq == args.eval_freq - 1 or epoch == args.epochs - 1:
            ...
            train_res_swag = utils.eval(loaders['train'], swag_model, criterion)
