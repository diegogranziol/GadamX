...
if args.num_curv_samples:
    ...
    extra = ''
args.dir = args.dir + args.dataset + '/' + args.model + '/Shrinkage_layerwise_SGD' + '/seed='+str(args.seed)+'_lr='+str(args.lr_init)+'_swastart=' +str(args.swag_start) + '_slr=' +str(args.swag_lr) + '_wdfreq='+str(args.wd_freq)+'_matrix='+ args.curvature_matrix + '_wdend=' +str(args.epochend) + extra + '_numepochs='+str(args.epochs)+ '/'
...
optimizer = ShrinkageOpt(
    [
        ...
        #This attempted edit removes batch normalisation from the shrinkage set
        for name,p in model.named_parameters() if 'bn' not in name
    ],
    lr=args.lr_init,
...
columns = ['ep', 'lr', 'tr_loss', 'tr_acc', 'te_loss', 'te_acc', 'norm', 'time', 'mem_usage']
if args.swag:
...
for epoch in range(start_epoch, args.epochs):
    ...
    values = [epoch + 1, lr, train_res['loss'], train_res['accuracy'], test_res['loss'],
              test_res['accuracy'], norm, time_ep, memory_usage]
    if args.swag:
