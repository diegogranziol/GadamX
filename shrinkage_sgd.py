...
args.dir = args.dir + args.dataset + '/' + args.model + '/Shrinkage_SGD' + '/seed='+str(args.seed)+'_lr='+str(args.lr_init)+'_swastart=' +str(args.swag_start) + '_slr=' +str(args.swag_lr) + '_wdfreq='+str(args.wd_freq)+'_matrix='+ args.curvature_matrix + '_wdend=' +str(args.epochend) + extra + '/'
...
columns = ['ep', 'wd', 'lr', 'tr_loss', 'tr_acc', 'te_loss', 'te_acc', 'te_top5_acc','alpha', 'mu', 'time', 'mem_usage']
if args.swag:
...
for epoch in range(start_epoch, args.epochs):
    ...
    if args.epochend is not None and epoch > args.epochend:
        ...
        if epoch % args.wd_freq == 0:
            ...
            loss_stats = utils.loss_stats(stats_loader, model, criterion, cuda=True, bn_train_mode=args.bn_train_mode, curvature_matrix=args.curvature_matrix)
    ...
    values = [epoch + 1,  wd, lr, train_res['loss'], train_res['accuracy'], test_res['loss'],
              test_res['accuracy'], test_res['top5_accuracy'], alpha, mu, time_ep, memory_usage]
